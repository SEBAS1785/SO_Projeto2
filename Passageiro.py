...
NOMES = [
    "António", "Ryan", "Tiago", "David", "Sebastian", "Fábio", "Guilherme",
...
def passenger_process(
    ...
    with queue_lock:
        ...
        # Ordena a lista partilhada para manter a prioridade
    ...
    while True:
        ...
        if passageiro.nome in result_dict:
            ...
            if resultado.get("desistiu"):
                # Servidor marcou como desistente
                logger.desistencia(passageiro.nome, espera)
                return
            else:
                # Embarque concluído (log já foi escrito pelo servidor)
                return

        # Timeout: passageiro desiste por conta própria
        if espera >= MAX_WAIT_TIME:
            ...
            # Marca desistência no result_dict para o servidor não embarcar
